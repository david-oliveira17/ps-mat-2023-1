...
const {Suplliers, Customer} = require('../models')
...
controller.create = async (req, res) => {
    try{
        await Suplliers.create(req.body)
        //HTTP 201: Created
        res.status(201).end()
    }
    catch(error){
...
controller.retrieve = async (req, res) => {
    try {
        const data = await Suplliers.findAll({
...
controller.retrieveOne = async (req, res) => {
    try {
        const data = await Suplliers.findByPk(req.params.id)
...
controller.update = async (req,res) => {
    try {
        const response = await Suplliers.update(
            req.body,
...
controller.delete = async (req, res) => {
    try {
        const response = await Suplliers.destroy(
            { where: {id: req.params.id} }
        )
